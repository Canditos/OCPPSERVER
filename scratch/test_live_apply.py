...
data = json.loads(res_create)
if "id" in data:
    profile_id = data["id"]
    print(f"Applying profile {profile_id} to JAAN462076...")
    cmd2 = f"curl -s -k -X POST https://ocpp.gatoescondido.com/api/smart-charging/apply -H 'Content-Type: application/json' -d '{{\"profile_id\": {profile_id}, \"charge_point_id\": \"JAAN462076\"}}'"
    stdin2, stdout2, stderr2 = ssh.exec_command(cmd2)
    res_apply = stdout2.read().decode('utf-8')
    print("=== APPLY PROFILE RESULT ===")
    print(res_apply)
