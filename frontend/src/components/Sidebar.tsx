...
import { Zap, LayoutDashboard, ArrowLeftRight, Terminal, Settings, Activity, ShieldCheck, Gauge, Sliders } from 'lucide-react'
    ...
    >
      ...
      <div className="p-3 m-3 space-y-2">
        <div className={`rounded-xl p-3 border space-y-2 ${
          ...
          <div className={`pt-2 border-t ${resolved === 'dark' ? 'border-white/5' : 'border-slate-200'}`}>
            <div className="flex items-center gap-1.5">
              <span className="relative flex h-1.5 w-1.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
                <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-400" />
              </span>
              <span className="text-xs text-emerald-500 font-medium">WebSocket activo</span>
            </div>
          </div>
        </div>
